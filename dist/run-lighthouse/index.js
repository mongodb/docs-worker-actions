...
const log = __nccwpck_require__(38237)('lhci:utils:api-client');
const fetch = __nccwpck_require__(92340);
...
exports = module.exports = __nccwpck_require__(22648);
exports.log = log;
...
exports.humanize = __nccwpck_require__(97882);
...
if (typeof process !== 'undefined' && process.type === 'renderer') {
  ...
  module.exports = __nccwpck_require__(72364);
}
...
var realFetch = __nccwpck_require__(80467);
...
if (
  ...
  exports.B1 = function (name) {
    throwIfEmpty(name);
    perf.mark(("start " + name));
  };
  exports.sT = function (name) {
    throwIfEmpty(name);
    perf.mark(("end " + name));
  ...
  exports.Cg = function () { return perf.getEntriesByType('measure'); };
  exports.ZH = function () {
    perf.clearMarks();
    perf.clearMeasures();
  };
  ...
  exports.Cg = function () { return entries; };
  exports.ZH = function () {
    marks = {};
    entries = [];
  };
...
const chromeLauncher = __importStar(__nccwpck_require__(73769));
...
const lh_server_1 = __nccwpck_require__(46780);
async function main() {
    ...
    const server = new lh_server_1.LHServer();
    ...
    const ancestorHash = branch === baseBranch
        ? (0, build_context_1.getAncestorHashForBase)()
        : (0, build_context_1.getAncestorHashForBranch)('HEAD', baseBranch);
    const buildInfo = {
        lifecycle: 'unsealed',
        hash: hash + Date.now(),
        branch,
        ancestorHash,
        commitMessage: (0, build_context_1.getCommitMessage)(hash),
        author: (0, build_context_1.getAuthor)(hash),
        avatarUrl: (0, build_context_1.getAvatarUrl)(hash),
        externalBuildUrl: (0, build_context_1.getExternalBuildUrl)(),
        runAt: new Date().toISOString(),
        committedAt: (0, build_context_1.getCommitTime)(hash),
        ancestorCommittedAt: ancestorHash ? (0, build_context_1.getCommitTime)(ancestorHash) : undefined,
    };
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.LHServer = void 0;
const report_generator_1 = __nccwpck_require__(63163);
const api_client_1 = __importDefault(__nccwpck_require__(6234));
/**
...
class LHServer {
    ...
    async listBuilds() {
        const builds = await this.api.getBuilds(this.project.id);
        console.log(builds);
    }
    async getIsNewBranch(buildInfo) {
        const builds = await this.api.getBuilds(this.project.id);
        const existingBranch = builds.find(build => build.branch === buildInfo.branch);
        return !existingBranch;
    }
    async deleteBuild(buildId) {
        await this.api.deleteBuild(this.project.id, buildId);
        console.log(`Deleted CI build (${buildId}) from CI project ${this.project.name} (${this.project.id})`);
    }
    async deleteAllBuilds() {
        const builds = await this.api.getBuilds(this.project.id);
        for (const build of builds) {
            await this.api.deleteBuild(this.project.id, build.id);
            console.log(`Deleted CI build (${build.id}) from CI project ${this.project.name} (${this.project.id})`);
        }
    }
    async createBuild(buildInfo) {
        const builds = await this.api.getBuilds(this.project.id);
        const existingBuild = builds.find(build => build.hash === buildInfo.hash);
        if (existingBuild) {
            console.log('Build exists - deleting');
            await this.api.deleteBuild(this.project.id, existingBuild.id);
        }
        const build = await this.api.createBuild({
            ...buildInfo,
            projectId: this.project.id,
        });
        console.log(`Saving CI build (${build.id}) to CI project ${this.project.name} (${this.project.id})`);
        return build;
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    async createRun(build, lhr, url) {
        const run = await this.api.createRun({
            projectId: this.project.id,
            buildId: build.id,
            representative: false,
            url,
            lhr: report_generator_1.ReportGenerator.generateReport(lhr, 'json'),
        });
        console.log(`Saving run for ${run.url} (${run.id})`);
        return run;
    }
    async sealBuild(build) {
        console.log(`Sealing CI build (${build.id})`);
        return await this.api.sealBuild(this.project.id, build.id);
    }
}
exports.LHServer = LHServer;
...
const lighthouse = async function lighthouse(...args) {
  const {default: lighthouse} = await Promise.all(/* import() */[__nccwpck_require__.e(404), __nccwpck_require__.e(623)]).then(__nccwpck_require__.bind(__nccwpck_require__, 58404));
  return lighthouse(...args);
};

lighthouse.startFlow = async function startFlow(...args) {
  const {startFlow} = await Promise.all(/* import() */[__nccwpck_require__.e(404), __nccwpck_require__.e(623)]).then(__nccwpck_require__.bind(__nccwpck_require__, 58404));
  return startFlow(...args);
};

lighthouse.navigation = async function navigation(...args) {
  const {navigation} = await Promise.all(/* import() */[__nccwpck_require__.e(404), __nccwpck_require__.e(623)]).then(__nccwpck_require__.bind(__nccwpck_require__, 58404));
  return navigation(...args);
};

lighthouse.startTimespan = async function startTimespan(...args) {
  const {startTimespan} = await Promise.all(/* import() */[__nccwpck_require__.e(404), __nccwpck_require__.e(623)]).then(__nccwpck_require__.bind(__nccwpck_require__, 58404));
  return startTimespan(...args);
};

lighthouse.snapshot = async function snapshot(...args) {
  const {snapshot} = await Promise.all(/* import() */[__nccwpck_require__.e(404), __nccwpck_require__.e(623)]).then(__nccwpck_require__.bind(__nccwpck_require__, 58404));
...
__nccwpck_require__.d(__webpack_exports__, {
  ...
  "getChromePath": () => (/* reexport */ getChromePath),
  "killAll": () => (/* reexport */ killAll),
  "launch": () => (/* reexport */ launch)
});

// NAMESPACE OBJECT: ./node_modules/chrome-launcher/dist/chrome-finder.js
var chrome_finder_namespaceObject = {};
__nccwpck_require__.r(chrome_finder_namespaceObject);
__nccwpck_require__.d(chrome_finder_namespaceObject, {
  "darwin": () => (darwin),
...
var external_net_ = __nccwpck_require__(41808);
...
var escape_string_regexp = __nccwpck_require__(98691);
...
// EXTERNAL MODULE: ./node_modules/chrome-launcher/node_modules/debug/src/index.js
var src = __nccwpck_require__(11728);
// EXTERNAL MODULE: ./node_modules/marky/lib/marky.cjs.js
var marky_cjs = __nccwpck_require__(44098);
;// CONCATENATED MODULE: ./node_modules/chrome-launcher/node_modules/lighthouse-logger/index.js
/**
...
const isWindows = external_process_.platform === 'win32';

// @ts-expect-error: process.browser is set via Rollup.
const isBrowser = external_process_.browser;
...
src.colors = [colors.cyan, colors.green, colors.blue, colors.magenta];

class Emitter extends external_events_.EventEmitter {
  /**
...
class Log {
  ...
  static setLevel(level) {
    ...
    switch (level) {
      ...
      case 'verbose':
        ...
        break;
      case 'warn':
        src.enable('-LH:*, LH:*:warn, LH:*:error');
        break;
      case 'error':
        src.enable('-LH:*, LH:*:error');
        break;
      default:
        src.enable('LH:*, -LH:*:verbose');
    }
  }
  ...
  static formatProtocol(prefix, data, level) {
    const columns = (!external_process_ || external_process_.browser) ? Infinity : external_process_.stdout.columns;
 ...
 */
Log.takeTimeEntries = () => {
  const entries = marky_cjs/* getEntries */.Cg();
  marky_cjs/* clear */.ZH();
  return entries;
};
 ...
 * @return {PerformanceEntry[]}
 */
Log.getTimeEntries = () => marky_cjs/* getEntries */.Cg();

/* harmony default export */ const lighthouse_logger = (Log);

// EXTERNAL MODULE: ./node_modules/is-wsl/index.js
var is_wsl = __nccwpck_require__(52559);
;// CONCATENATED MODULE: ./node_modules/chrome-launcher/dist/utils.js
/**
...
function getPlatform() {
    return is_wsl ? 'wsl' : process.platform;
}
function makeTmpDir() {
    switch (getPlatform()) {
        case 'darwin':
...
function toWin32Path(dir = '') {
    ...
    try {
        return external_child_process_.execFileSync('wslpath', ['-w', dir]).toString().trim();
    }
    catch {
...
function toWSLPath(dir, fallback) {
    try {
        return external_child_process_.execFileSync('wslpath', ['-u', dir]).toString().trim();
    }
    catch {
        return fallback;
    }
...
function makeUnixTmpDir() {
    return external_child_process_.execSync('mktemp -d -t lighthouse.XXXXXXX').toString().trim();
}
function makeWin32TmpDir() {
    const winTmpPath = process.env.TEMP || process.env.TMP ||
        (process.env.SystemRoot || process.env.windir) + '\\temp';
    const randomNumber = Math.floor(Math.random() * 9e7 + 1e7);
    const tmpdir = (0,external_path_.join)(winTmpPath, 'lighthouse.' + randomNumber);
    (0,external_fs_.mkdirSync)(tmpdir, { recursive: true });
    return tmpdir;
}
...
;// CONCATENATED MODULE: ./node_modules/chrome-launcher/dist/chrome-finder.js
/**
...
function darwin() {
    ...
    (0,external_child_process_.execSync)(`${LSREGISTER} -dump` +
        ...
        suffixes.forEach(suffix => {
            const execPath = external_path_.join(inst.substring(0, inst.indexOf('.app') + 4).trim(), suffix);
    ...
    // clang-format off
    const home = escape_string_regexp(process.env.HOME || (0,external_os_.homedir)());
    const priorities = [
    ...
    if (process.env.LIGHTHOUSE_CHROMIUM_PATH) {
        priorities.unshift({ regex: new RegExp(escape_string_regexp(process.env.LIGHTHOUSE_CHROMIUM_PATH)), weight: 150 });
    }
    if (process.env.CHROME_PATH) {
        priorities.unshift({ regex: new RegExp(escape_string_regexp(process.env.CHROME_PATH)), weight: 151 });
    }
    // clang-format on
...
function resolveChromePath() {
    ...
    if (canAccess(process.env.LIGHTHOUSE_CHROMIUM_PATH)) {
        lighthouse_logger.warn('ChromeLauncher', 'LIGHTHOUSE_CHROMIUM_PATH is deprecated, use CHROME_PATH env variable instead.');
...
function linux() {
    ...
    const desktopInstallationFolders = [
        external_path_.join((0,external_os_.homedir)(), '.local/share/applications/'),
    ...
    executables.forEach((executable) => {
        try {
            const chromePath = (0,external_child_process_.execFileSync)('which', [executable], { stdio: 'pipe' }).toString().split(newLineRegex)[0];
    ...
    if (process.env.LIGHTHOUSE_CHROMIUM_PATH) {
        priorities.unshift({ regex: new RegExp(escape_string_regexp(process.env.LIGHTHOUSE_CHROMIUM_PATH)), weight: 100 });
    }
    if (process.env.CHROME_PATH) {
        priorities.unshift({ regex: new RegExp(escape_string_regexp(process.env.CHROME_PATH)), weight: 101 });
    }
...
function win32() {
    const installations = [];
    const suffixes = [
        `${external_path_.sep}Google${external_path_.sep}Chrome SxS${external_path_.sep}Application${external_path_.sep}chrome.exe`,
        `${external_path_.sep}Google${external_path_.sep}Chrome${external_path_.sep}Application${external_path_.sep}chrome.exe`
    ];
    const prefixes = [
    ...
    prefixes.forEach(prefix => suffixes.forEach(suffix => {
        const chromePath = external_path_.join(prefix, suffix);
        if (canAccess(chromePath)) {
            installations.push(chromePath);
        }
    }));
...
function findChromeExecutables(folder) {
    ...
    if (canAccess(folder)) {
        ...
        try {
            execPaths = (0,external_child_process_.execSync)(`grep -ER "${chromeExecRegex}" ${folder} | awk -F '=' '{print $2}'`, { stdio: 'pipe' });
        }
        catch (e) {
            execPaths = (0,external_child_process_.execSync)(`grep -Er "${chromeExecRegex}" ${folder} | awk -F '=' '{print $2}'`, { stdio: 'pipe' });
        }
...
;// CONCATENATED MODULE: ./node_modules/chrome-launcher/dist/random-port.js
/**
 * @license Copyright 2016 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with the License. You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0
 * Unless required by applicable law or agreed to in writing, software distributed under the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the License for the specific language governing permissions and limitations under the License.
...
function getRandomPort() {
    return new Promise((resolve, reject) => {
        const server = (0,external_http_.createServer)();
        server.listen(0);
...
;// CONCATENATED MODULE: ./node_modules/chrome-launcher/dist/flags.js
/**
 * @license Copyright 2017 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with the License. You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0
 * Unless required by applicable law or agreed to in writing, software distributed under the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the License for the specific language governing permissions and limitations under the License.
...
;// CONCATENATED MODULE: ./node_modules/chrome-launcher/dist/chrome-launcher.js
/**
 * @license Copyright 2016 Google Inc. All Rights Reserved.
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except in compliance with the License. You may obtain a copy of the License at http://www.apache.org/licenses/LICENSE-2.0
 * Unless required by applicable law or agreed to in writing, software distributed under the License is distributed on an "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. See the License for the specific language governing permissions and limitations under the License.
...
const chrome_launcher_isWindows = getPlatform() === 'win32';
...
class Launcher {
    constructor(opts = {}, moduleOverrides = {}) {
        ...
        this.fs = moduleOverrides.fs || external_fs_namespaceObject;
        this.spawn = moduleOverrides.spawn || external_child_process_.spawn;
        lighthouse_logger.setLevel(defaults(this.opts.logLevel, 'silent'));
    ...
    static getFirstInstallation() {
        if (getPlatform() === 'darwin')
            return darwinFast();
        return chrome_finder_namespaceObject[getPlatform()]()[0];
    }
    /** Returns all available chrome installations in decreasing priority order. */
    static getInstallations() {
        return chrome_finder_namespaceObject[getPlatform()]();
    }
    ...
    prepare() {
        ...
        lighthouse_logger.verbose('ChromeLauncher', `created ${this.userDataDir}`);
    ...
    setBrowserPrefs() {
        ...
        try {
            ...
            lighthouse_logger.log('ChromeLauncher', `Failed to set browser prefs: ${err.message}`);
        }
    }
    async launch() {
        if (this.requestedPort !== 0) {
            ...
            try {
                ...
                lighthouse_logger.log('ChromeLauncher', `Found existing Chrome already running using port ${this.port}, using that.`);
                return;
            }
            catch (err) {
                ...
                lighthouse_logger.log('ChromeLauncher', `No debugging port found on port ${this.port}, launching a new Chrome.`);
            }
        }
    ...
    async spawnProcess(execPath) {
        const spawnPromise = (async () => {
            if (this.chromeProcess) {
                lighthouse_logger.log('ChromeLauncher', `Chrome already running with pid ${this.chromeProcess.pid}.`);
            ...
            lighthouse_logger.verbose('ChromeLauncher', `Launching with command:\n"${execPath}" ${this.flags.join(' ')}`);
            ...
            lighthouse_logger.verbose('ChromeLauncher', `Chrome running with pid ${this.chromeProcess.pid} on port ${this.port}.`);
            return this.chromeProcess.pid;
        })();
    ...
    isDebuggerReady() {
        return new Promise((resolve, reject) => {
            const client = external_net_.createConnection(this.port, '127.0.0.1');
    ...
    waitUntilReady() {
        ...
        return new Promise((resolve, reject) => {
            ...
            const poll = () => {
                if (retries === 0) {
                    lighthouse_logger.log('ChromeLauncher', waitStatus);
                }
                retries++;
                waitStatus += '..';
                lighthouse_logger.log('ChromeLauncher', waitStatus);
                launcher.isDebuggerReady()
                    .then(() => {
                    lighthouse_logger.log('ChromeLauncher', waitStatus + `${lighthouse_logger.greenify(lighthouse_logger.tick)}`);
                    resolve();
                })
                    .catch(err => {
                    if (retries > launcher.maxConnectionRetries) {
                        lighthouse_logger.error('ChromeLauncher', err.message);
                        const stderr = this.fs.readFileSync(`${this.userDataDir}/chrome-err.log`, { encoding: 'utf-8' });
                        lighthouse_logger.error('ChromeLauncher', `Logging contents of ${this.userDataDir}/chrome-err.log`);
                        lighthouse_logger.error('ChromeLauncher', stderr);
                        return reject(err);
                    }
    ...
    kill() {
        ...
        lighthouse_logger.log('ChromeLauncher', `Killing Chrome instance ${this.chromeProcess.pid}`);
        try {
            if (chrome_launcher_isWindows) {
                // https://github.com/GoogleChrome/chrome-launcher/issues/266
                const taskkillProc = (0,external_child_process_.spawnSync)(`taskkill /pid ${this.chromeProcess.pid} /T /F`, { shell: true, encoding: 'utf-8' });
                const { stderr } = taskkillProc;
                if (stderr)
                    lighthouse_logger.error('ChromeLauncher', `taskkill stderr`, stderr);
            }
            else {
                if (this.chromeProcess.pid) {
            ...
            lighthouse_logger.warn('ChromeLauncher', message);
        }
        this.destroyTmp();
    }
    destroyTmp() {
...
;
/* harmony default export */ const chrome_launcher = ((/* unused pure expression or super */ null && (Launcher)));
...
;// CONCATENATED MODULE: ./node_modules/chrome-launcher/dist/index.js
...
// EXPORTS
__nccwpck_require__.d(__webpack_exports__, {
  "ReportGenerator": () => (/* binding */ ReportGenerator)
});

// EXTERNAL MODULE: external "fs"
var external_fs_ = __nccwpck_require__(57147);
// EXTERNAL MODULE: ./node_modules/lighthouse/shared/esm-utils.js
var esm_utils = __nccwpck_require__(55580);
...
const moduleDir = (0,esm_utils/* getModuleDirectory */.oJ)(import.meta);

/* eslint-disable max-len */
const FLOW_REPORT_TEMPLATE = external_fs_.readFileSync(`${moduleDir}/../../flow-report/assets/standalone-flow-template.html`, 'utf8');
const REGULAR_REPORT_CSS = external_fs_.readFileSync(moduleDir + '/../assets/styles.css', 'utf8');
const FLOW_REPORT_CSS = external_fs_.readFileSync(`${moduleDir}/../../flow-report/assets/styles.css`, 'utf8');
const FLOW_REPORT_JAVASCRIPT = external_fs_.readFileSync(`${moduleDir}/../../dist/report/flow.js`, 'utf8');
...
const report_assets_moduleDir = (0,esm_utils/* getModuleDirectory */.oJ)(import.meta);
...
"use strict";
/* harmony export */ __nccwpck_require__.d(__webpack_exports__, {
/* harmony export */   "EU": () => (/* binding */ getModulePath),
/* harmony export */   "oJ": () => (/* binding */ getModuleDirectory)
/* harmony export */ });
/* unused harmony export resolveModulePath */
/* harmony import */ var module__WEBPACK_IMPORTED_MODULE_0__ = __nccwpck_require__(98188);
/* harmony import */ var url__WEBPACK_IMPORTED_MODULE_1__ = __nccwpck_require__(57310);
...
function getModulePath(importMeta) {
  return url__WEBPACK_IMPORTED_MODULE_1__.fileURLToPath(importMeta.url);
}
...
function getModuleDirectory(importMeta) {
  return path__WEBPACK_IMPORTED_MODULE_2__.dirname(getModulePath(importMeta));
}
...
/******/ 	var __webpack_exports__ = __nccwpck_require__(__nccwpck_require__.s = 77428);
